...
var cssmw1 = {
	...
	testPageLocation: function(testLoc) {
		...
		if (hrefString.indexOf("://") >= 0) {
			...
			if (hrefString.charAt(0) == "index.html") {
				hrefString = hrefString.substring(1);
			}
			hrefString = hrefString.substring(hrefString.indexOf("index.html"));
		}
		if (testLoc.indexOf("://") >= 0) {
			testLoc = testLoc.substring(testLoc.indexOf("://") + 3);
			if (testLoc.charAt(0) == "index.html") {
				testLoc = testLoc.substring(1);
			}
			testLoc = testLoc.substring(testLoc.indexOf("index.html"));
		}
		var hrefFolder = hrefString.substring(0, hrefString.lastIndexOf("index.html") + 1);
		var testFolder = testLoc.substring(0, testLoc.lastIndexOf("index.html") + 1);
